    ...
    bool is_valid_word() {
        for (int i = 0; i < this->rows; i++) {
            ...
            for (int j = 0; j < this->columns; j++) {
                ...
                rev_row_word += this->board[i][j];
                rev_col_word += this->board[j][i];
            }
            swap(rev_row_word[0],rev_row_word[2]);
            swap(rev_col_word[0],rev_col_word[2]);

            if (find(dictionary.begin(), dictionary.end(), row_word) != dictionary.end()) {
                winning_word = row_word;
                return true;
            }
            if (find(dictionary.begin(),dictionary.end(), rev_row_word) != dictionary.end()) {
                winning_word = rev_row_word;
                return true;
            }
            ...
            if (find(dictionary.begin(), dictionary.end(), rev_col_word) != dictionary.end()) {
                winning_word = rev_col_word;
                return true;
            }
        }
        ...
        string diag1_word = "";
        string diag2_word = "";
        string rev_diag1_word = "";
        string rev_diag2_word = "";
        for (int i = 0; i < this->rows; i++) {
            diag1_word += this->board[i][i];
            diag2_word += this->board[i][this->columns - i - 1];
            rev_diag1_word += this->board[i][i];
            rev_diag2_word += this->board[i][this->columns - i - 1];
        }
        swap(rev_diag1_word[0],rev_diag1_word[2]);
        swap(rev_diag2_word[0],rev_diag2_word[2]);

        if (find(dictionary.begin(), dictionary.end(), diag1_word) != dictionary.end()) {
            winning_word = diag1_word;
            return true;
        }
        if (find(dictionary.begin(), dictionary.end(), rev_diag1_word) != dictionary.end()) {
            winning_word = rev_diag1_word;
            return true;
        }
        ...
        if (find(dictionary.begin(), dictionary.end(), rev_diag2_word) != dictionary.end()) {
            winning_word = rev_diag2_word;
            return true;
        }
        return false;
    }
